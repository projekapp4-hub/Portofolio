...
const mVisitLink = document.getElementById('modal-visit-link'); // Tambahkan ini
const mTechStack = document.getElementById('modal-tech-stack'); // Tambahkan ini (opsional)
...
function openModal(data) {
    ...
    // Set link untuk project (dengan validasi)
    if (data.link && mVisitLink) {
        mVisitLink.href = data.link;
        mVisitLink.style.display = 'inline-flex'; // Tampilkan link
    } else if (mVisitLink) {
        mVisitLink.style.display = 'none'; // Sembunyikan jika tidak ada link
    }
    
    // Set technology tags (opsional)
    if (data.tech && mTechStack) {
        const techArray = data.tech.split(',');
        mTechStack.innerHTML = techArray.map(tech => 
            `<span class="px-3 py-1 bg-slate-100 dark:bg-white/10 rounded-full text-xs font-medium text-slate-600 dark:text-slate-300">${tech.trim()}</span>`
        ).join('');
    }
...
triggers.forEach(trigger => {
    trigger.addEventListener('click', () => {
        const data = {
            ...
            link: trigger.dataset.link,        // Tambahkan ini - link spesifik project
            tech: trigger.dataset.tech          // Tambahkan ini - teknologi yang digunakan
        };
        openModal(data);
    });
});
